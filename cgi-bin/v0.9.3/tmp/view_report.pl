...
if ( $is_finished != '-1' ) { print '<div style="text-align:center"><h2 style="color:red;">Warning: Data analysis not finished ' . $is_finished . '</h2></div>'; }

my $top_hits = $results_dbh->prepare("SELECT * FROM results WHERE name=? AND fragment LIKE '%-%' ORDER BY score DESC");    #nice injection problem here, need to sort
$top_hits->execute($table);
print_report( $top_hits, $mass_of_hydrogen, $mass_of_deuterium, $mass_of_carbon12, $mass_of_carbon13, $cut_residues, $protein_sequences, $reactive_site, $results_dbh, $xlinker_mass, $mono_mass_diff, $table, 1 );
