...
sub import_cgi_query {
    ...
    if (defined $query->param('aions')) { $ms2_fragmentation{'aions'} = '1' } else { $ms2_fragmentation{'aions'} = '0' }
    if (defined $query->param('bions')) { $ms2_fragmentation{'bions'} = '1' } else { $ms2_fragmentation{'bions'} = '0' }
    if (defined $query->param('yions')) { $ms2_fragmentation{'yions'} = '1' } else { $ms2_fragmentation{'yions'} = '0' }

    if (defined $query->param('waterloss'))   { $ms2_fragmentation{'waterloss'}   = '1' } else { $ms2_fragmentation{'waterloss'}   = '0' }
    if (defined $query->param('ammonialoss')) { $ms2_fragmentation{'ammonialoss'} = '1' } else { $ms2_fragmentation{'ammonialoss'} = '0' }
