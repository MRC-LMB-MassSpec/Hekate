...
my $top_hits = $results_dbh->prepare("SELECT * FROM  results WHERE name=? AND SCORE > 0 AND scan= ? AND fraction = ? ORDER BY score DESC");    #nice injection problem here, need to sort
