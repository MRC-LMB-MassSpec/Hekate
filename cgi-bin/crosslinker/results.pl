...
while (my $table_name = $table_list->fetchrow_hashref) {
    ...
    if ($table_name->{'finished'} != -1 && $table_name->{'finished'} != -4 && $table_name->{'finished'} != -5) {
        print '<iframe style="border: 0px; width:5em; height:1.25em; overflow-y: hidden;" src="status-iframe.pl?table=',
...
print
'<div style="width: 20em; margin: auto; padding:1em;"> <input type="submit" value="Combine"> - <a href="view_log.pl">View Log</a>';
