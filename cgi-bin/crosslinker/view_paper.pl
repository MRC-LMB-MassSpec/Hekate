...
sub print_results_paper {
   ...
   if ( $no_tables == 0 ) {
      print
'<table><tr><td></td><td>Chain 1</td><td>Chain 2</td><td>Position1</td><td>Position2</td><td>Fragment&nbsp;and&nbsp;Position</td><td>Score</td><td>Charge</td><td>Mass<td>PPM</td></td></td><td>Mod</td></tr>';
   }
        ...
        )
      {
         ...
         if ( $top_hits_results->{'fragment'} =~ '-' ) {
            if ( substr( $top_hits_results->{'sequence1_name'}, 1 ) lt substr( $top_hits_results->{'sequence2_name'}, 1 ) ) {
               ...
               $flip_order = 1;
            } elsif ( substr( $top_hits_results->{'sequence1_name'}, 1 ) eq substr( $top_hits_results->{'sequence2_name'}, 1 )
                      && residue_position( $unmodified_fragments[0], $protien_sequences ) > residue_position( $unmodified_fragments[1], $protien_sequences ) )
            {
               $flip_order = 1;
               print "<td>",        substr( $top_hits_results->{'sequence2_name'}, 1 );
               print " </td><td> ", substr( $top_hits_results->{'sequence1_name'}, 1 );
         ...
         if ( $top_hits_results->{'fragment'} =~ '-' ) {
            ...
            if ( $flip_order == 0 ) {
               print residue_position ( $unmodified_fragments[0], $protien_sequences ) + $top_hits_results->{'best_x'} + 1 +
                 $error{ substr( $top_hits_results->{'sequence1_name'}, 1 ) };
               print "</td><td>";    #, $fragments[0], "&#8209;";
               print residue_position ( $unmodified_fragments[1], $protien_sequences ) + $top_hits_results->{'best_y'} + 1 +
