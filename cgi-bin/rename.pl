...
if ( defined $new_name ) {
    ...
    my $table_list = $settings_dbh->prepare("SELECT name, desc, finished FROM settings WHERE name=? ORDER BY length(name) DESC, name DESC");
	...
	New name: <input type='text' name='name' value='" . $table_name->{'desc'} . "' />
